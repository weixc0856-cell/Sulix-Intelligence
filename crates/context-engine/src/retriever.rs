...
pub async fn retrieve_decisions<S: StoreBackend>(
    ...
    let scored: Vec<ScoredDecision> = decisions.into_iter().filter_map(|d| {
        ...
        Some(ScoredDecision {
            ...
            rank_components: crate::types::RankComponents {
                query_alignment: 0.0, confidence: d.confidence, recency: 0.0, usage_frequency: 0.0, user_specificity: 1.0,
            },
        })
    }).collect();
    Ok(scored)
...
pub async fn retrieve_memories<S: StoreBackend>(
    store: &S, query: &MemoryQuery,
) -> Result<Vec<ScoredMemory>, String> {
    let memories = store.list_memories(None, query.status.as_deref(), query.limit).await.map_err(|e| format!("list_memories: {e}"))?;
    let scored: Vec<ScoredMemory> = memories.into_iter().filter_map(|m| {
        Some(ScoredMemory {
            ...
            relevance_score: 0.0,
            rank_components: crate::types::RankComponents {
                query_alignment: 0.0, confidence: m.confidence, recency: 0.0, usage_frequency: m.usage_count as f64, user_specificity: 1.0,
            },
        })
    }).collect();
    Ok(scored)
