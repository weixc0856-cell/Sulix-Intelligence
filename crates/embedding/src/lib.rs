...
impl EmbeddingProvider for WorkersAiEmbedder {
    async fn embed(&self, text: &str) -> Result<Vec<f32>, EmbeddingError> {
        let ai = self.env.ai("AI").map_err(|e| EmbeddingError::Request(e.to_string()))?;
        ...
        let result: serde_json::Value = ai
            .run("@cf/baai/bge-large-en-v1.5", BgeInput { text: vec![text.to_string()] })
            .await
            .map_err(|e| EmbeddingError::Request(e.to_string()))?;

        let embedding =
            result["data"][0].as_array().ok_or_else(|| EmbeddingError::Response("missing data[0]".into()))?;
