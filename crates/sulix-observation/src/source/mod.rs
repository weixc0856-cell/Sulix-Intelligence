...
/// 专题聚合/紧急加更配置（从旧 domain::SpecialTopic 迁移至此）
...
pub fn load_special_topics(vault_path: &str) -> Vec<SpecialTopic> {
    ...
    if let Ok(entries) = std::fs::read_dir(&flash_path) {
        for entry in entries.flatten() {
            ...
            if let Ok(content) = std::fs::read_to_string(&path) {
                if let Ok(topic) = serde_json::from_str::<SpecialTopic>(&content) {
                    topics.push(topic);
                }
            }
        }
