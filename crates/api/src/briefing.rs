...
struct BriefingArtifactEnvelope {
    ...
    /// The inner briefing JSON (schema matches what `intelligence_briefs.content` stores).
    content: serde_json::Value,
}

/// Read today's briefing from the R2 Memory Archive.
///
/// Returns `Some(value)` when the object exists and passes validation,
/// `None` when missing, and falls back on error.
