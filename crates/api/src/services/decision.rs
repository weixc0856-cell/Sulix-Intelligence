...
//! evaluation) goes through this service, which atomically:
//! 1. Mutates D1 state via StoreBackend
//! 2. Appends an event to the EventStore (R2 Memory Archive)

use event_store::{AggregateRef, EventEnvelope, EventMetadata, EventStore, keys as event_keys};
use store::{Decision, NewDecision, NewDecisionEvaluation, NewOutcomeEvent, StoreBackend, StoreError};
...
pub struct DecisionService<S: StoreBackend, E: EventStore> {
    store: S,
    event_store: E,
}

impl<S: StoreBackend, E: EventStore> DecisionService<S, E> {
    pub fn new(store: S, event_store: E) -> Self {
    ...
    /// Create a new decision and append a DecisionCreated event.
    ...
    /// Change decision status and append a DecisionStatusChanged event.
    ...
    /// Emits DecisionStatusChanged + OutcomeObserved (independent aggregate).
    ...
    /// Record an evaluation and append a DecisionEvaluated event.
