...
pub async fn list(req: Request, ctx: RouteContext<ProductionAppServices>) -> Result<Response> {
    ...
    match service.list(source_type, source_id, limit, offset).await {
...
pub async fn lineage(_req: Request, ctx: RouteContext<ProductionAppServices>) -> Result<Response> {
    ...
    match service.lineage(id).await {
        Ok(Some((observation, source))) => response::json_ok(json!({ "observation": observation, "source": source })),
        Ok(None) => response::json_err(404, "observation not found"),
        Err(e) => {
            console_log!("[Sulix:observations] lineage get observation failed: {e}");
            response::json_err_internal("lineage query failed")
        }
    }
}
