...
impl crate::D1Store {
    ...
    pub async fn get_signal_briefing_context_bundle(
        ...
        let entity_rows: Vec<EntityRow> = self.db.prepare(&entity_sql).bind(&binds)?.all().await?.results()?;
        ...
        let decision_rows: Vec<DecisionRow> = self.db.prepare(&decision_sql).bind(&binds)?.all().await?.results()?;
        ...
        if !decision_ids.is_empty() {
            ...
            let eval_rows: Vec<EvalRow> = self.db.prepare(&eval_sql).bind(&eval_binds)?.all().await?.results()?;
