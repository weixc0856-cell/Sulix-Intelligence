...
impl D1Store {
    ...
    pub async fn get_feed(&self, id: i64) -> Result<Option<Feed>, StoreError> {
        Ok(self.db.prepare("SELECT id, url, title, category, fetch_interval_sec, last_fetched_at, etag, last_modified, status, extraction_level FROM feeds WHERE id = ?1").bind(&[JsValue::from_f64(id as f64)])?.first::<Feed>(None).await?)
    }
    ...
    pub async fn insert_feed(
        ...
        let row = self.db.prepare("INSERT OR IGNORE INTO feeds (url, title, category, fetch_interval_sec) VALUES (?1, ?2, ?3, ?4) RETURNING id").bind(&[url.into(), title.into(), category.into(), JsValue::from_f64(interval as f64)])?.first::<serde_json::Value>(None).await?;
    ...
    pub async fn record_fetch_result(
        ...
        self.db.prepare("UPDATE feeds SET last_fetched_at = ?1, etag = COALESCE(?2, etag), last_modified = COALESCE(?3, last_modified) WHERE id = ?4")
            .bind(&[JsValue::from_f64(fetched_at as f64), etag.map_or(JsValue::null(), |v| v.into()), last_modified.map_or(JsValue::null(), |v| v.into()), JsValue::from_f64(feed_id as f64)])?.run().await?;
        Ok(())
    }
}
