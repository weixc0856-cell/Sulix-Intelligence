...
impl D1Store {
    ...
    pub async fn health_stats(&self) -> Result<HealthStats, StoreError> {
        ...
        ).first::<HealthStats>(None).await.s_err()?.ok_or_else(|| StoreError::D1("health_stats returned no row".into()))
    }
    ...
    pub async fn score_distribution(&self) -> Result<ScoreDist, StoreError> {
        ...
        ).first::<ScoreDist>(None).await.s_err()?.unwrap_or(ScoreDist { top: 0, medium: 0, low: 0, unscored: 0 }))
    }
    ...
    pub async fn expired_article_r2_keys(&self, now: i64, days: i64) -> Result<Vec<String>, StoreError> {
        ...
        let rows: Vec<serde_json::Value> = self.db.prepare("SELECT raw_content_r2_key FROM articles WHERE published_at < ?1 AND ai_summary != '' AND ai_summary IS NOT NULL AND raw_content_r2_key IS NOT NULL").bind(&[JsValue::from_f64(cutoff as f64)]).s_err()?.all().await.s_err()?.results().s_err()?;
    ...
    pub async fn article_ids_exist(&self, ids: &[i64]) -> Result<std::collections::HashSet<i64>, StoreError> {
        ...
        stmt = stmt.bind(&vals).s_err()?;
        let rows: Vec<serde_json::Value> = stmt.all().await.s_err()?.results().s_err()?;
