...
async fn generate_asi_and_premium(
    ...
    Vec<crate::domain::PremiumReport>,
) {
...
async fn run_change_detection(
    ...
    let recent_entries: Vec<ChronicleEntry> = state
        .chronicle
        .as_ref()
...
async fn refine_domains(
    themes: &[Theme],
    config: &Config,
    api_key: &str,
) -> HashMap<String, (StrategicDomain, Vec<StrategicDomain>)> {
    let mut refined_domains: HashMap<String, (StrategicDomain, Vec<StrategicDomain>)> =
        HashMap::new();
    for theme in themes {
        let text = format!(
            "{} {}",
            ...
            match crate::llm::call_and_parse(api_key, &config.llm, system_prompt, &text).await {
                Ok(response) if crate::domain::StrategicDomain::validate_llm_output(&response) => {
                    let (primary, secondary) =
                        crate::domain::StrategicDomain::parse_llm_response(&response);
                    log::info!(
                        "🧠 Domain classify [{}]: {} (LLM-refined)",
                        theme.title,
                        primary.label()
                    );
    ...
    if !refined_domains.is_empty() {
        log::info!(
            "🧠 Domain classification: {} topics LLM-refined",
            refined_domains.len()
        );
    }
    refined_domains
...
pub async fn publish_generate(
    ...
    // 1. ASI 评分 + Premium 研报
    let (asi_score_map, premium_reports) =
        generate_asi_and_premium(themes, analyses, config, api_key, today).await;
    ...
    // 6. Editor Notes
    let editor_notes =
        crate::agent::editor::analyze_personal_impact(analyses, state.memory_for_linking.theses());
