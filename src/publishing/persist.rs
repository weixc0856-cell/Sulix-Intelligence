...
pub async fn publish_persist(
    ...
    if let Err(e) = state.registry.save(&state.registry_path) {
        ...
        log::info!(
            "📋 Assessment Registry: {} assessments, next ID: ASM-{:04}",
            state.registry.assessments.len(),
            state.registry.core.next_id
        );
    }

    // Decision Registry save
    let dec_registry_path = PathBuf::from(&config.output.vault_path).join("decision_registry.json");
    let mut dec_registry =
        crate::engine::decision_registry::DecisionRegistry::load_or_new(&dec_registry_path);
    {
        let theses_snapshot: Vec<_> = inferred
            .memory
            .theses()
            .iter()
        ...
        for td in &inferred.thesis_decisions {
            if let Some((_, Some(asm_id))) =
                theses_snapshot.iter().find(|(id, _)| id == &td.thesis_id)
            {
                if let Some(event) =
                    inferred
                        .memory
                        .record_or_update_decision(td, asm_id, today, &mut dec_registry)
                {
    ...
    if let Err(e) = dec_registry.save(&dec_registry_path) {
        ...
        log::info!(
            "🎯 Decision Registry: {} decisions, next ID: DEC-{:04}",
            dec_registry.decisions.len(),
            dec_registry.core.next_id
        );
    }
    ...
    if let Err(e) = state.inv_registry.save(&state.inv_registry_path) {
        ...
        log::info!(
            "📋 Investigation Registry: {} investigations, next ID: INV-{:04}",
            state.inv_registry.investigations.len(),
            state.inv_registry.core.next_id
        );
    }
    ...
    // SQLite report
    if let Err(e) = db.record_report(
        today,
        &format!("Daily brief - {} topics", inferred.memory.theses().len()),
        0,
    ) {
    ...
    log::info!(
        "📊 Registry counts: ASM={}, INV={}, DEC={}, OUT={}",
        state.registry.assessments.len(),
        state.inv_registry.investigations.len(),
        dec_registry.decisions.len(),
