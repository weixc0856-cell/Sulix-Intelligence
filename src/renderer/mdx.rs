...
use crate::engine::memory::{Thesis, Outcome, Reflection, ThesisStatus, Stance};
...
pub fn render_daily_mdx(
    ...
    let sources_yaml = sources.iter()
        .map(|s| format!("  - {}", yaml_escape(s)))
        .collect::<Vec<_>>()
        .join("\n");

    let entities: Vec<String> = analysis.fact_base.iter()
        .flat_map(|fb| fb.evidence.split_whitespace())
        .map(|w| w.to_uppercase())
        .filter(|w| ["TSMC", "ASML", "NVIDIA", "OPENAI", "ANTHROPIC", "GOOGLE", "META", "MICROSOFT", "INTEL", "AMD", "ARM", "HBM"].contains(&w.as_str()))
        .collect::<std::collections::BTreeSet<_>>()
        .into_iter()
        .collect();
    let entities_yaml = entities.iter()
        .map(|e| format!("  - {}", e))
        .collect::<Vec<_>>()
        .join("\n");

    // Editor notes for this theme
    let theme_notes: Vec<&crate::agent::editor::EditorNote> = editor_notes.iter()
    ...
    if !analysis.assumptions.is_empty() {
        ...
        for a in &analysis.assumptions {
            ...
            mdx.push_str(&format!("    evidence_strength: {}\n", yaml_escape(&a.evidence_strength)));
        }
    }
    mdx.push_str("---\n\n");
...
pub fn render_thesis_mdx(thesis: &Thesis, outcomes: &[Outcome]) -> String {
    ...
    if !thesis.evidences.is_empty() {
        ...
        for e in thesis.evidences.iter().rev().take(20) {
            ...
            mdx.push_str(&format!("| {} | {} | {} | {} |\n",
                e.date, yaml_escape(&e.title), icon, yaml_escape(&e.summary)));
        }
        mdx.push('\n');
    }
...
pub fn render_reflection_mdx(reflection: &Reflection, thesis_title: &str) -> String {
    ...
    mdx.push_str(&format!("## Error Analysis\n\n{}\n\n", reflection.error_reason));
